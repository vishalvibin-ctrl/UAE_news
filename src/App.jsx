...
const cleanTitle=(t)=>{if(!t)return"";if(t.includes(" - ")){const p=t.split(" - ");p.pop();return p.join(" - ").trim();}return t.trim();};
const extractSource=(title,source)=>{if(source)return source;if(title?.includes(" - ")){const p=title.split(" - ");return p[p.length-1].trim();}return"News";};
...
const fetchFeed=async(feedUrl)=>{
  ...
  try{
    ...
    if(res.ok){const data=await res.json();if(data.status==="ok"&&data.items?.length>0){const items=data.items.map(item=>({title:cleanTitle(item.title),description:(item.description||"").slice(0,300),link:item.link,pubDate:item.pubDate,image:item.image||null,source:extractSource(item.title,item.source)}));CACHE[ck]={data:items,ts:Date.now()};return items;}}
  ...
  try{
    ...
    if(res.ok){const data=await res.json();if(data.status==="ok"&&data.items?.length>0){const items=data.items.map(item=>({title:cleanTitle((item.title||"").replace(/<[^>]*>/g,"")),description:(item.description||item.content||"").replace(/<[^>]*>/g,"").slice(0,300),link:item.link,pubDate:item.pubDate,image:item.thumbnail||null,source:extractSource(item.title,item.author)}));CACHE[ck]={data:items,ts:Date.now()};return items;}}
...
const ThreatGauge=({newsCount})=>{
  ...
  return(
    <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:20,padding:"24px 20px",marginBottom:20}}>
...
const StoryChart=({news})=>{
  ...
  return(
    <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:20,padding:"24px 20px",marginBottom:20}}>
      <div style={{fontSize:13,color:"#8B95A5",fontWeight:500,letterSpacing:0.5,marginBottom:16}}>Coverage by Source</div>
...
// ═══════════════════════════════════════════
// REEL SLIDE — content starts from top area
// ═══════════════════════════════════════════
  ...
  return(
    <div style={{height:"100dvh",width:"100%",scrollSnapAlign:"start",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",top:24,left:20,right:20,zIndex:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{padding:"4px 12px",borderRadius:20,background:"rgba(0,0,0,0.45)",backdropFilter:"blur(10px)",fontSize:12,color:"rgba(255,255,255,0.7)",fontWeight:500}}>
          {index+1} / {total}
        </div>
        <div style={{padding:"4px 12px",borderRadius:20,background:"rgba(0,0,0,0.45)",backdropFilter:"blur(10px)",fontSize:11,color:"#C8A050",fontWeight:500}}>
          {timeAgo(item.pubDate)}
        </div>
      </div>

      {/* Main content — starts at top third */}
      <div style={{position:"absolute",top:70,left:0,right:0,bottom:80,zIndex:5,display:"flex",flexDirection:"column",justifyContent:"flex-start",padding:"20px 24px 0",overflow:"hidden"}}>
        ...
        {/* Title */}
        <h2 style={{fontSize:28,fontWeight:700,lineHeight:1.22,color:"#FFF",margin:"0 0 18px",fontFamily:"'Newsreader',Georgia,serif",letterSpacing:-0.3,textShadow:"0 2px 20px rgba(0,0,0,0.5)"}}>
          {item.title}
        </h2>
        ...
        {item.description?.length>20&&(
          <p style={{fontSize:15,lineHeight:1.6,color:"rgba(255,255,255,0.7)",margin:"0 0 24px",display:"-webkit-box",WebkitLineClamp:5,WebkitBoxOrient:"vertical",overflow:"hidden",textShadow:"0 1px 8px rgba(0,0,0,0.3)"}}>
            {item.description}
          </p>
        )}
...
// ═══════════════════════════════════════════
// TAB BAR — compact, 4 tabs with icons only on mobile
// ═══════════════════════════════════════════
...
const TabBar=({tab,setTab,floating,sr})=>(
  <div style={{display:"flex",gap:4,borderRadius:floating?16:12,background:floating?"rgba(20,24,36,0.9)":"rgba(255,255,255,0.03)",backdropFilter:floating?"blur(20px)":"none",border:floating?"1px solid rgba(255,255,255,0.06)":"none",padding:4}}>
    {TABS.map(t=>{
      const active=tab===t.id;
      const isRed=t.id==="security"&&active;
      return(
        <button key={t.id} onClick={()=>{setTab(t.id);if(sr?.current)sr.current.scrollTop=0;}} style={{
          flex:1,padding:floating?"10px 4px":"10px 2px",background:active?(isRed?"rgba(244,67,54,0.12)":"rgba(200,160,80,0.12)"):"transparent",
          border:"none",borderRadius:floating?12:10,cursor:"pointer",
          fontSize:11,fontWeight:active?600:400,
          color:active?(isRed?"#F44336":"#C8A050"):"#5A6475",
              ...
              <>
                ...
                {!isLd&&cur.length>0&&(
                  <>
                    ...
                    <StoryChart news={cur}/>
                    <div style={{fontSize:13,color:"#8B95A5",fontWeight:500,letterSpacing:0.5,marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
